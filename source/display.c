...
//    C3D_FVUnifMtx4x4(GPU_VERTEX_SHADER, uLoc_projection, &projection);
    ...
    screen_draw_texture(TEXTURE_SCREEN_TOP_SPLASH_BG, (TOP_SCREEN_WIDTH - topScreenBgWidth) / 2, (TOP_SCREEN_HEIGHT - topScreenBgHeight) / 2, topScreenBgWidth, topScreenBgHeight);
    
    screen_get_texture_size(&bannerWidth, &bannerHeight, TEXTURE_APP_BANNER);
    screen_draw_texture(TEXTURE_APP_BANNER, (TOP_SCREEN_WIDTH - bannerWidth) / 2, (TOP_SCREEN_HEIGHT - bannerHeight) / 2, bannerWidth, bannerHeight);
}
